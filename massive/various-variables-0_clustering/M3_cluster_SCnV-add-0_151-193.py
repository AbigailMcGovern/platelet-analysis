...
sp = os.path.join(d, f'211206_saline_df_spherical-coords_151-193_{dt}.parquet')

eps_list=(0.1, 0.15, 0.2)
cols = ('rho', 'theta', 'phi', 'rho_diff', 'theta_diff', 'phi_diff', 'frame', 'nb_density_15', 'nrterm', 'ca_corr', 'cont', 'dv') #, 'path_len', 'disp', 'ca_corr')
df = umap_dbscan_cluster(df, 'SCnV-add-0', save=sp, eps_list=eps_list, cols=cols)
df.to_parquet(sp)

# save plots here
save_dir = f'/fs02/rl54/clustering/{dt}_UMAP-DBSCAN_SCnV-add-0_151-193'
...
for col in cluster_cols:
    n = dt + '_' + col + '_151-193_scatter.pdf'
...
multi_basic_scatter_save(df, umap_cols[0], umap_cols[1], cols, save_dir, f'UMAP-DBSCAN_SCnV-add-0_151-193')
