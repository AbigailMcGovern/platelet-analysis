...
df = df[df['path'] != '200527_IVMTR73_Inj5_saline_exp3']# 200527_IVMTR73_Inj5_saline_exp3
...
sp = os.path.join(d, f'211206_saline_df_spherical-coords_{dt}.parquet')

eps_list=(0.1, 0.15, 0.2)
cols = ('rho_diff', 'theta_diff', 'phi_diff', 'dv', 'nd15_percentile') # here density percentile is serving as the only positional information, the rest is movement
df = umap_dbscan_cluster(df, 'SCnV-add-0', save=sp, eps_list=eps_list, cols=cols)
df.to_parquet(sp)

# save plots here
save_dir = f'/fs02/rl54/clustering/{dt}_UMAP-DBSCAN_SCnV-add-0'
os.makedirs(save_dir, exist_ok=True)

# build and save the cluster plots
umap_cols = ['umap_0_SCnV-add-0', 'umap_1_SCnV-add-0']
cluster_cols = [f'SCnV-add-0_udbscan_{e}' for e in eps_list]
...
multi_basic_scatter_save(df, umap_cols[0], umap_cols[1], cols, save_dir, f'UMAP-DBSCAN_SCnV-add-0')
