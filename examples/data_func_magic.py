...
def build_df_lists(col_list,treatments):#Builds dataframe from lists of variables and list (inh_order) with treatments
    ...
    if '_veh-mips_' in inh_list:
        print('Adding xtra experiments to Vehicle MIPS',flush=True)
        xtra_list= [path for path in paths if 'salgav-veh' in path.name]
        list1=list1+xtra_list
        if '_salgav-veh_' not in inh_list:
            #print('Removing xtra experiments to Vehicle MIPS')
            remove_exp=True
        else:
            remove_exp=False
    else:
    ...
    if xtra_list:
        if remove_exp:
            remove_ls=['210603_IVMTR116_Inj5_SalgavDMSO_exp3','210603_IVMTR116_Inj6_SalgavDMSO_exp3',
                       '210609_IVMTR120_Inj5_SalgavDMSO_exp3','210609_IVMTR120_Inj6_SalgavDMSO_exp3',
                       '210609_IVMTR120_Inj8_SalgavDMSO_exp3','210611_IVMTR122_Inj8_SalgavDMSO_exp3',
                       '210611_IVMTR122_Inj9_SalgavDMSO_exp3']
            pc.loc[pc.inh=='_salgav-veh_','inh']='_veh-mips_'
            pc=pc[~(pc.path.isin(remove_ls))]
        else:
            add_ls=['210721_IVMTR129_Inj7_galsavDMSO_exp3','210721_IVMTR129_Inj8_galsavDMSO_exp3',
                    '210723_IVMTR131_Inj4_gavsalDMSO_exp3','210723_IVMTR131_Inj5_gavsalDMSO_exp3',
                    '210723_IVMTR131_Inj6_gavsalDMSO_exp3']
            xtra_pc=pc[pc.path.isin(add_ls)].copy()
            xtra_pc['inh']='_veh-mips_'
            pc=pc.append(xtra_pc)
    
    for inh in pc.inh.unique():
        pc.loc[pc.inh==inh,'inh']=cfg.shorttolong_dic[inh]
